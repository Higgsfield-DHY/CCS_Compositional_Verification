...
public class NewObserbationTable implements Learner<ResetLogicTimeWord> {
    ...
    public void show() {
        ...
        for (ResetLogicTimeWord word : prefixList) {
            String s = DisplayRenderUtil.renderResetWord(word);
            stringList.add(s);
            maxLen = Math.max(maxLen, s.length());
        }
        for (LogicTimeWord words : suffixSet) {
            String s = DisplayRenderUtil.renderLogicWord(words);
        ...
        for (int i = 0; i < prefixList.size(); i++) {
            ...
            if (fatherRow != null) {
                sb.append(DisplayRenderUtil.renderResetWord(
                        recorder.childFatherMap.get(row(prefixList.get(i))).getPrefix()));
            }
