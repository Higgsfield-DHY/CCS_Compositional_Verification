...
public class NTA {
    ...
    private boolean checkIgnore(SynchronizedLabel syncLable) {
        ...
        if (ignoreSigma.contains(labelText)) {
            return false;
        }
        if (labelText != null && labelText.length() >= 2) {
            char suffix = labelText.charAt(labelText.length() - 1);
            if (suffix == '!' || suffix == '?') {
                String base = labelText.substring(0, labelText.length() - 1);
                return !ignoreSigma.contains(base);
            }
        }
        return !ignoreSigma.contains(labelText);
    }
}
