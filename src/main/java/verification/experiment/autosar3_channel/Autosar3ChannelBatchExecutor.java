...
public class Autosar3ChannelBatchExecutor {
    public static void main(String[] args) throws IOException {
        ...
        if (verboseCaseId != null && Autosar3ChannelCases.findById(verboseCaseId) == null) {
            throw new IllegalArgumentException("Unknown verbose case: " + verboseCaseId
                    + ". Available: " + Autosar3ChannelCases.availableCases());
        }

        List<Autosar3ChannelCase> selected = new ArrayList<Autosar3ChannelCase>();
        if (runAll) {
            selected.addAll(Autosar3ChannelCases.allCases());
        } else {
            ...
            Autosar3ChannelCase scenario = Autosar3ChannelCases.findById(selectedCaseId);
            if (scenario == null) {
        ...
        List<Autosar3ChannelRunner.RunSummary> rows = new ArrayList<Autosar3ChannelRunner.RunSummary>();
        boolean allPass = true;
        for (Autosar3ChannelCase scenario : selected) {
            boolean detailed = verboseCaseId != null && scenario.getCaseId().equalsIgnoreCase(verboseCaseId);
            Autosar3ChannelRunner.RunSummary summary;
            if (detailed) {
                System.out.println();
                System.out.println("----- Detailed log start: " + scenario.getCaseId() + " -----");
                summary = Autosar3ChannelRunner.runCaseDetailed(scenario, timeoutMs);
                System.out.println("----- Detailed log end: " + scenario.getCaseId() + " -----");
                System.out.println();
            } else {
                summary = Autosar3ChannelRunner.runCase(scenario, timeoutMs);
            }
            rows.add(summary);
        ...
        printTable(rows);
        if (!allPass) {
            System.err.println("AUTOSAR-3 Channel batch validation failed.");
            System.exit(1);
        }
    }
    ...
    private static void printCasePurpose(List<Autosar3ChannelCase> cases) {
        System.out.println("AUTOSAR-3 Channel run list:");
        for (Autosar3ChannelCase c : cases) {
    ...
    private static void printTable(List<Autosar3ChannelRunner.RunSummary> rows) {
        String format = "%-20s %-18s %-16s %-8s %-8s %-9s %-10s %-10s %-10s %s%n";
        System.out.printf(format,
                "Case", "Structure", "Partition", "Direct", "AG", "Status", "Direct(ms)", "AG(ms)", "Timeout", "Result");
        System.out.printf(format,
                "--------------------", "------------------", "----------------",
                "--------", "--------", "--------", "----------", "----------", "----------", "------");
        for (Autosar3ChannelRunner.RunSummary row : rows) {
