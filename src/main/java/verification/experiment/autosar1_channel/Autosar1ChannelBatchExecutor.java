...
public class Autosar1ChannelBatchExecutor {
    public static void main(String[] args) throws IOException {
        ...
        if (verboseCaseId != null && Autosar1ChannelCases.findById(verboseCaseId) == null) {
            throw new IllegalArgumentException("Unknown verbose case: " + verboseCaseId
                    + ". Available: " + Autosar1ChannelCases.availableCases());
        }

        List<Autosar1ChannelCase> selected = new ArrayList<Autosar1ChannelCase>();
        if (runAll) {
            selected.addAll(Autosar1ChannelCases.allCases());
        } else {
            ...
            Autosar1ChannelCase scenario = Autosar1ChannelCases.findById(selectedCaseId);
            if (scenario == null) {
        ...
        List<Autosar1ChannelRunner.RunSummary> rows = new ArrayList<Autosar1ChannelRunner.RunSummary>();
        boolean allPass = true;
        for (Autosar1ChannelCase scenario : selected) {
            boolean detailed = verboseCaseId != null && scenario.getCaseId().equalsIgnoreCase(verboseCaseId);
            Autosar1ChannelRunner.RunSummary summary;
            if (detailed) {
                System.out.println();
                System.out.println("----- Detailed log start: " + scenario.getCaseId() + " -----");
                summary = Autosar1ChannelRunner.runCaseDetailed(scenario, timeoutMs);
                System.out.println("----- Detailed log end: " + scenario.getCaseId() + " -----");
                System.out.println();
            } else {
                summary = Autosar1ChannelRunner.runCase(scenario, timeoutMs);
            }
            rows.add(summary);
        ...
        printTable(rows);
        if (!allPass) {
            System.err.println("AUTOSAR-1 Channel batch validation failed.");
            System.exit(1);
        }
    }
    ...
    private static void printCasePurpose(List<Autosar1ChannelCase> cases) {
        System.out.println("AUTOSAR-1 Channel run list:");
        for (Autosar1ChannelCase c : cases) {
    ...
    private static void printTable(List<Autosar1ChannelRunner.RunSummary> rows) {
        String format = "%-22s %-18s %-16s %-8s %-8s %-9s %-10s %-10s %-10s %s%n";
        System.out.printf(format,
                "Case", "Structure", "Partition", "Direct", "AG", "Status", "Direct(ms)", "AG(ms)", "Timeout", "Result");
        System.out.printf(format,
                "----------------------", "------------------", "----------------",
                "--------", "--------", "--------", "----------", "----------", "----------", "------");
        for (Autosar1ChannelRunner.RunSummary row : rows) {
